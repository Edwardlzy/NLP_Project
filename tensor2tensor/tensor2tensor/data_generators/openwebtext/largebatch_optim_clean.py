...
class LargebatchOptimizer(optimizer.Optimizer):
  ...
  def __init__(self, optimizer, update_steps, use_locking=False, name="Largebatch"):
    ...
    tf.logging.info("update_steps = %d", update_steps)
  ...
  def _prepare(self):
    ...
    self._update_steps = len(self._var_list) * self._update_steps  # make sure each var is visited update_steps times before applying.
